...
local function main(argc, argv)
  ...
  local err = ffi.new("DBusError");

  -- Message to display.
  --local dispMsg = "Hello World!";
  local dispMsg = ffi.new("const char *[1]", ffi.cast("const char *", "Hello World!"));

  -- Text to use for the acknowledgement button. "" means default.
  --local  buttonText = "";
  local buttonText = ffi.new("const char *[1]", ffi.cast("const char *", ""));
  ...
  dbus.dbus_message_set_no_reply(msg, TRUE);
